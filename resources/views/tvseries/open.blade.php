    ...
    <div class="ms-5 pt-5 pb-5">
        ...
        <div class="mt-3 d-flex flex-row">
            ...
            <div class="ms-5 me-5" style="width: 60%">
                ...
                @if (!$isOnWatchlist)
                <form action="/tvserie/{{$id}}/addtowatchlist" method="POST">
                    @csrf
                    <input type="hidden" name="tvseries" id="tvseries" value="tvseries">
                    <input type="hidden" name="itemid" id="itemid" value="{{$id}}">
                    <button type="submit" class="btn btn-primary">Add to watchlist</button>
                </form>
                @else
                <form action="/tvserie/{{$id}}/deletefromwatchlist" method="POST">
                    @csrf
                    <input type="hidden" name="tvseries" id="tvseries" value="tvseries">
                    <input type="hidden" name="itemid" id="itemid" value="{{$id}}">
